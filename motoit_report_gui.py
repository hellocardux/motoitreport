# cardux_motorbike_gui.py
# GUI per avviare scraping Moto.it, vedere progresso e aprire il report HTML
import re
import time
import threading
...
APP_CREDIT = "@fuori.tempo.massimo"
...
def discover_pages(base_url: str, headers: Dict[str, str], max_pages: int, delay: float, log=None) -> List[str]:
    ...
    try:
        ...
        if log: log(f"Impossibile stimare la paginazione: {e}. Procedo incrementale.")
...
def run_scrape(cfg: ScrapeConfig, progress_cb=None, log=None) -> pd.DataFrame:
    ...
    for idx, p in enumerate(pages, start=1):
        if log: log(f"Pagina {idx}/{total}: {p}")
...
class App(tk.Tk):
    ...
    def _build_header(self):
        ...
        ttk.Label(header, text="Comparativa usato Moto.it con report interattivo", style="Sub.TLabel", foreground="#666").pack(side="right")
    ...
    def _build_form(self):
        ...
        # riga 4
        ttk.Label(grid, text="Cartella output").grid(row=3, column=0, sticky="w", padx=(0,8), pady=4)
        row4 = ttk.Frame(grid)
        row4.grid(row=3, column=1, columnspan=3, sticky="we", pady=4)
    ...
    def _build_footer(self):
        ...
        ttk.Label(footer, text=APP_CREDIT, style="Footer.TLabel", anchor="center").pack(fill="x")
    ...
    def on_run(self):
        ...
        t = threading.Thread(
            ...
            args=(brand, model, url, pages, delay, outdir),
            daemon=True
        )
        t.start()

    def _do_work(self, brand, model, url, pages, delay, outdir: Path):
        try:
            search_url = url or build_url(brand, model)
    ...
    def open_folder(self):
        ...
        try:
            ...
            webbrowser.open(str(folder.as_uri()))
